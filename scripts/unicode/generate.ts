...
import {
  ...
  parseNameAliases,
  parseRangeValueFile,
  parseSelectedProperties,
  parseUnicodeData,
  readVendoredFile,
  resetBlockOutputDirectory,
  slugify,
  sortByCodePoint,
  toSearchRecord,
  writeCompactJson,
...
const createBlockEntries = (records: CharacterRecord[]): BlockIndexEntry[] => {
  const blockMap = new Map<string, CharacterRecord[]>()
...
const createSearchPayload = (records: CharacterRecord[]): SearchPayload => {
  ...
  const encodedRecords = visibleSearchRecords.map((record: SearchRecord) => {
...
const run = async (): Promise<void> => {
  ...
  const characterRecords = sortByCodePoint(
    buildCharacterRecords(parsedRecords, {
